## 2026-07-22 - [In-Memory Rate Limiter Bottlenecks]
**Learning:** Checking rate limits using in-memory dicts with full scans for eviction on every single request leads to severe performance regressions (scaling linearly O(N) with the number of unique IPs). Using list comprehensions for window pruning is also slow.
**Action:** Decouple global eviction/cleanup logic to run periodically (e.g. every 10s) instead of on every request. Track timestamps with `collections.deque` and use `popleft()` for efficient O(1) sliding window pruning.
