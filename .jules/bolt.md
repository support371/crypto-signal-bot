...
**Learning:** Functions that calculate a full time series only to return the last value (e.g., ) introduce significant memory and CPU overhead due to list allocation and redundant calculations. For $ candles, this is (N)$ space when (1)$ is possible.
**Action:** Always implement optimized "last-value" versions of technical indicators that skip list allocation and calculate the target value in a single pass. For Bollinger Bands, (period)$ is sufficient for the last bar, avoiding the (N)$ rolling sum logic.

## 2026-06-03 - [MACD Redundant Series Calculation]
**Learning:** Signal engines often need both current and previous bar values for crossover detection. Calling  twice results in two full series calculations ( \times 2$ EMAs).
**Action:** Extend "last-value" indicators with a `count` parameter to allow fetching the last $ values in a single pass, drastically reducing the number of EMA calculations in strategy hot paths.
