...
def last_ema(values: List[float], period: int) -> Optional[float]:
    ...
    Further optimized by using itertools.islice to eliminate indexing overhead.
    """
    ...
    # Using islice to avoid indexing in the hot loop
...
def last_rsi(values: List[float], period: int = 14) -> Optional[float]:
    ...
    prev = values[0]
    # Seeding loop optimized with islice
    for curr in itertools.islice(values, 1, period + 1):
        change = curr - prev
        if change > 0:
            avg_gain += change
        else:
    ...
    # Wilder smoothing for the rest, optimized with islice
...
def last_atr(
    ...
    # Using zip and islice to avoid indexing
    it_h = itertools.islice(highs, 1, period + 1)
    it_l = itertools.islice(lows, 1, period + 1)
    it_pc = itertools.islice(closes, 0, period)
