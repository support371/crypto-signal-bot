...
_rate_limit_store: Dict[str, collections.deque[float]] = {}
...
def _rate_limit_memory(client_ip: str) -> None:
    ...
    Optimized for high concurrency and performance:
      - Replaced list-based sliding window with collections.deque to achieve O(1) removals.
      - Uses lazy pruning (popleft) on every access to avoid O(N) list reallocations.
      - Decoupled the global stale IP scan to run periodically (every 10s) instead of
        on every request, avoiding O(U) dictionary scans on the hot path.
    """
    ...
    with _store_lock:
        # 1. Periodic global cleanup of stale client IPs to prevent memory leaks
        # We decouple this heavy O(U) scan to run at most once every 10 seconds.
        ...
        # 2. Get or initialize the sliding window deque for this client IP
        if client_ip not in _rate_limit_store:
            _rate_limit_store[client_ip] = collections.deque()

        timestamps = _rate_limit_store[client_ip]

        # 3. Lazy pruning of stale timestamps for the current client IP (O(1) popleft)
