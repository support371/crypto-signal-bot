...
const bitgetPreview = read('worker/src/live/adapters/bitget/preview.ts')
...
requirePattern(bitgetPreview, /previewSource:\s*'LOCAL_LOCKED_ESTIMATE'/, 'Bitget preview must be explicitly local and locked', failures)
requirePattern(bitgetPreview, /executionAllowed:\s*false/, 'Bitget preview must never allow execution', failures)
requirePattern(bitgetPreview, /CandidateExecutionLockedError\('bitget\.createOrder'\)/, 'Bitget create-order preview adapter lock is missing', failures)
requirePattern(bitgetPreview, /CandidateExecutionLockedError\('bitget\.requestWithdrawal'\)/, 'Bitget withdrawal preview adapter lock is missing', failures)
requirePattern(bitgetPreview, /local_estimate_not_exchange_guarantee/, 'Bitget local-preview warning is missing', failures)

requirePattern(decimal, /bigint/, 'exact decimal arithmetic must be BigInt-backed', failures)
requirePattern(decimal, /divideDecimalDown/, 'exact downward-rounded division is missing', failures)
