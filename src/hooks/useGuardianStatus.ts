...
import { normalizeGuardianStatus, type WorkerGuardianStatus } from '@/lib/guardianStatus';
...
export function useGuardianStatus(pollIntervalMs = 15000) {
  ...
  const fetch = useCallback(async () => {
    try {
      const data = await fetchBackendJson<WorkerGuardianStatus>('/guardian/status');
      setGuardian(normalizeGuardianStatus(data));
    } catch {
