...
export function Header({
  ...
  const demoLiveMismatch = demoMode && backendConnected && systemMode === 'live';
  ...
  const statusDotClass = !backendConnected
    ? 'bg-muted-foreground'
    : killSwitchActive || demoLiveMismatch
    ? 'bg-destructive'
    : 'bg-accent';
  ...
  const subtitle = !backendConnected
    ...
    : demoLiveMismatch
    ? 'Demo UI connected to live backend — verify deployment settings'
    : systemMode === 'live'
  ...
  return (
    <>
      {demoLiveMismatch && (
        <div className="bg-destructive/90 text-white py-2 px-4 text-center font-mono text-xs border-b border-destructive">
          <span className="font-bold">SAFETY WARNING:</span> VITE_DEMO_MODE is enabled but the backend reports LIVE mode. Keep manual orders as paper-only until the backend is reset to paper/testnet or live mode is intentionally certified.
        </div>
      )}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <div className={`absolute -top-1 -right-1 w-3 h-3 rounded-full ${statusDotClass} ${backendConnected && !killSwitchActive && !demoLiveMismatch ? 'animate-pulse-glow' : ''}`} />
              </div>
              <div>
            ...
            <div className="flex items-center gap-4">
              <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted/50 border border-border">
                <div className={`w-2 h-2 rounded-full ${statusDotClass} ${backendConnected && !killSwitchActive && !demoLiveMismatch ? 'animate-pulse' : ''}`} />
